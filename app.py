...
def app(song_list_file):
    ...
    for line in song_list_file:
        ...
        try:
            ...
            click.echo(f"line=`{line}` got error: {rte}", err=True)
            pass
...
def youtube_search_first_hit(query):
    ...
    # - items > id > videoId
    # - items > snippet > title
    search_url = f'{api_url}?part=snippet&fields=items(id,snippet(title))&maxResults={max_results}&q={encoded_query}&type=video&key={API_KEY}'
    headers = {'Accept': 'application/json'}
    response = requests.get(search_url, headers=headers)
    response_json = response.json()
    if response.status_code != 200:
        raise RuntimeError(f"error={response_json['error']}")
